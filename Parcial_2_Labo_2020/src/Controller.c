...
*/
/*
 * \brief 	Alta de cliente, agrega un nuevo cliente a la lista con todos sus campos rellenados por el usuario
...
/*
 * \brief 	Alta de ventas, agrega un nuevo venta a la lista con todos sus campos rellenados por el usuario
 * \param 	LinkedList* this
 * \return 	int -1 ERROR si la this == NULL /
 * 			int -1 ERROR si no se pudieron obtener los datos del empleado correctamente /
 * 			int -1 ERROR si el pCliente == NULL
...
/*
 * \brief 	Busca los maximos y minimos del cliente que vendio mas afiches, segun el filtro pasado como parametro
 * \param 	LinkedList* thisA
 * \param 	LinkedList* thisB
 * \param 	int filtro
 * \return 	int -1 ERROR this == NULL || path == NULL /
 * 			int 0
 */
...
/*
 * \brief 	Busca los maximos y minimos de la venta que vendio mas afiches
 * \param 	LinkedList* thisA
 * \param 	LinkedList* thisB
 * \return 	int -1 ERROR this == NULL || path == NULL /
 * 			int 0
...
/*
 * \brief edita una venta seleccionada con el id pedido y que tenga su estado en ACOBRAR
 * \param LinkedList* thisA
 * \param LinkedList* thisB
 * \param int caso
 * \return	int -1 ERROR VALIDACIONES
 * 			int 0 OK
 */
...
int controller_editVentasConParametro(LinkedList* thisA, LinkedList* thisB, int caso )
{
	...
	if(thisA != NULL && thisB != NULL)
	{
		listaSinCobrar = ll_clone(thisA); //CLONO LISTA VENTAS
		if(ll_filter(listaSinCobrar, soloSinCobrar) == 0)
		{
			...
			if( lengthSinCobrar > 0)
			{
				...
				if(ll_map(listaSinCobrar, ventas_imprimirListaACobrar)==0)
				{
					printf("\nOK PRINT\n");
					if(utn_getNumberInt("Ingrese el Id de la venta que desea modificar:\n", "Error, ingrese un valor numérico\n", &idVentaPedido, 3, 1, 100)==0)
					{
						...
						if(idClienteAImprimir > -1)
						{
							...
							{
								printf("ERROR EN EL PARAMETRO CASO\n");
							}
